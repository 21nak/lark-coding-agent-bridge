...
默认情况下，每个 profile 都使用当前 profile 的 lark-cli 目录：`~/.lark-channel/profiles/<profile>/lark-cli`。agent 子进程会收到指向这个目录的 `LARKSUITE_CLI_CONFIG_DIR`，所以一个 profile 里的个人授权不会共享给另一个 profile。

如果希望和本机终端共用同一份 lark-cli 默认配置及个人登录，可以在 `~/.lark-channel/config.json` 中把当前 profile 的 `larkCli.configSource` 设为 `local`，然后重启 bridge：
...
local 模式会直接使用 `~/.lark-cli/config.json`，且不注入 `LARK_CHANNEL=1`；否则 lark-cli 会自动切到独立的 `~/.lark-cli/lark-channel/config.json` 工作区。本地 lark-cli 配置的 App 必须与 bridge App 一致。所有使用 local 模式的 bridge profile 会共享同一份 lark-cli 身份和授权状态。
...
| `~/.lark-cli/` | 本机 lark-cli 默认目录；`larkCli.configSource` 为 `local` 时使用 |
